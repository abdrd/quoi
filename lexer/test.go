...
func compareResults(want, result string) bool {
	type diff struct {
	...
	if !(bothReachedEOF) {
		log.Println("[!!!] Different number of line counts")
		return false
	}
	...
	writeFile("diff.tokens", diffStr.String())
	return len(diffs) == 0
}
...
func runTest() {
	...
	writeFile("result.tokens", res.String())
	ok := compareResults(readFile("wanted.tokens"), readFile("result.tokens"))
	if ok {
		log.Println("[✊] PASS :)")
	}
}
